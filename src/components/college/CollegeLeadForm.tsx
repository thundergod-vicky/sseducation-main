...
export function CollegeLeadForm({ collegeShort }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    googleFormData.append("entry.85122333", `College Page (${collegeShort}) - Preferred Branch: ${parsed.data.branch}`);
    ...
    try {
      ...
      setSubmitting(false);
      setSubmitted(true);
      toast.success(`Thank you! Our ${collegeShort} counsellor will call you within 30 minutes.`);
    } catch (error) {
      setSubmitting(false);
      ...
      <div className="relative">
        <div className="mb-5 text-center">
          ...
          <h3 className="mt-3 text-2xl font-bold text-foreground leading-tight">
            Book Free {collegeShort} Counselling
          </h3>
          <p className="text-sm text-muted-foreground mt-1">
            Get instant seat & fee guidance for B.Tech
          </p>
        </div>

        <div className="space-y-3.5">
          <div>
            <Label htmlFor={`${collegeShort}-name`} className="text-xs font-semibold">Full Name *</Label>
            <Input id={`${collegeShort}-name`} name="name" placeholder="Your full name" className="mt-1" />
            {errors.name && <p className="text-xs text-destructive mt-1">{errors.name}</p>}
          </div>
          <div>
            <Label htmlFor={`${collegeShort}-phone`} className="text-xs font-semibold">Mobile Number *</Label>
            <Input id={`${collegeShort}-phone`} name="phone" type="tel" placeholder="10-digit mobile" maxLength={10} className="mt-1" />
            {errors.phone && <p className="text-xs text-destructive mt-1">{errors.phone}</p>}
          </div>
          <div>
            <Label htmlFor={`${collegeShort}-email`} className="text-xs font-semibold">Email (optional)</Label>
            <Input id={`${collegeShort}-email`} name="email" type="email" placeholder="name@example.com" className="mt-1" />
            {errors.email && <p className="text-xs text-destructive mt-1">{errors.email}</p>}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor={`${collegeShort}-state`} className="text-xs font-semibold">State *</Label>
              <Select name="state">
                <SelectTrigger id={`${collegeShort}-state`} className="mt-1"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>
            ...
            <div>
              <Label htmlFor={`${collegeShort}-branch`} className="text-xs font-semibold">Branch *</Label>
              <Select name="branch">
                <SelectTrigger id={`${collegeShort}-branch`} className="mt-1"><SelectValue placeholder="Select" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="CSE">CSE</SelectItem>
                  <SelectItem value="AI/ML">AI / ML</SelectItem>
                  <SelectItem value="ECE">ECE</SelectItem>
                  <SelectItem value="ISE">ISE / IT</SelectItem>
